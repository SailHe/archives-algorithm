...
// ---- 根据进程索引查询进程
// 返回进程在进程表中的索引 若不存在返回负数
int findProcess(int procIdx) {
	int i, reIndex = -1;
	for (i = 0; i < MAX_PROC && Processes[i].ProcIndex > 0; ++i) {
		...
		for (i = 0; i < N; i++)
			for (j = 0; j < N; j++){
				if (bitMap.map[i][j] == 0){
					...
					printf("行: %d, 列: %d, 块号: %d\n", i, j, blockNum);
					/* 填充进程页表 */
					Processes[procNum].ProcPageTable[subProcPageTable++] = blockNum;
					/* 修改位示图该位的标记，为1表示已被占用 */
					bitMap.map[i][j] = 1;
